<div align="center">

# 👋 Hi, I'm Hasnain

### Full Stack Software Engineer

### Building Modern Software • Enterprise Solutions • AI-Assisted Development

<img src="https://readme-typing-svg.herokuapp.com?font=Poppins&weight=600&size=24&pause=1200&color=38BDF8&center=true&vCenter=true&width=850&lines=Full+Stack+Software+Engineer;ASP.NET+Core+%7C+Laravel+%7C+React+Developer;Flutter+Developer;Building+Enterprise+Applications;Leveraging+AI+to+Build+Smarter+Software;Always+Learning+%F0%9F%9A%80"/>

<br>

<img src="https://komarev.com/ghpvc/?username=HasnainAfzaal&style=for-the-badge&color=0ea5e9"/>

<img src="https://img.shields.io/github/followers/HasnainAfzaal?style=for-the-badge"/>

<img src="https://img.shields.io/github/stars/HasnainAfzaal?style=for-the-badge"/>

</div>

---

# 🚀 About Me

I'm a **Full Stack Software Engineer** passionate about building scalable, secure and high-performance applications.

I specialize in developing **Enterprise Software**, **Business Solutions**, **REST APIs**, **POS Systems**, **Inventory Management Systems** and **Modern Web Applications**.

I embrace **AI-assisted software development**, using tools like **Cursor**, **Claude Code**, **ChatGPT** and **GitHub Copilot** to accelerate development, improve code quality and streamline engineering workflows—while ensuring every solution is designed, reviewed and validated with software engineering best practices.

---

# ⚙️ Tech Stack

### Backend

<p>
<img src="https://skillicons.dev/icons?i=dotnet,cs,php,laravel,nodejs"/>
</p>

* ASP.NET Core
* C#
* .NET
* PHP
* Laravel
* REST APIs
* Entity Framework Core
* MVC Architecture

---

### Frontend

<p>
<img src="https://skillicons.dev/icons?i=react,js,html,css,bootstrap,tailwind"/>
</p>

* React
* JavaScript (ES6+)
* HTML5
* CSS3
* Bootstrap
* Tailwind CSS

---

### Mobile Development

<p>
<img src="https://skillicons.dev/icons?i=flutter,firebase"/>
</p>

* Flutter
* Firebase

---

### Databases

<p>
<img src="https://skillicons.dev/icons?i=mysql,mongodb"/>
</p>

* Microsoft SQL Server
* MySQL
* MongoDB

---

### Cloud

<p>
<img src="https://skillicons.dev/icons?i=azure"/>
</p>

**Currently Learning**

* Next.js
* Azure Cloud Services
* System Design
* Microservices

---

### Development Tools

<p>
<img src="https://skillicons.dev/icons?i=git,github,vscode,visualstudio,postman"/>
</p>

* Git
* GitHub
* Visual Studio
* Visual Studio Code
* Postman
* cPanel

---

# 🤖 AI Development Workflow

Modern software development is no longer just about writing code—it's about delivering better solutions efficiently.

I use AI as a development accelerator to:

* Speed up development
* Improve code quality
* Generate technical documentation
* Debug complex issues
* Review and refactor code
* Explore new technologies faster
* Increase development productivity

### AI Tools

* ChatGPT
* Claude Code
* Cursor AI
* GitHub Copilot
* Prompt Engineering

---

# 💼 Areas of Expertise

* Enterprise Software
* Business Applications
* ERP Solutions
* POS Systems
* Inventory Management
* E-Commerce Platforms
* REST APIs
* Admin Dashboards
* SaaS Applications
* Responsive Websites
* Mobile Applications

---

# 📊 GitHub Analytics

<img height="170" src="https://github-readme-stats.vercel.app/api?username=HasnainAfzaal&show_icons=true&theme=tokyonight"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=HasnainAfzaal&layout=compact&theme=tokyonight"/>

---

# 🔥 Contribution Streak

<img src="https://streak-stats.demolab.com?user=HasnainAfzaal&theme=tokyonight"/>

---

# 📈 Contribution Graph

<img src="https://github-readme-activity-graph.vercel.app/graph?username=HasnainAfzaal&theme=tokyo-night"/>

---

# 🏆 GitHub Achievements

<img src="https://github-profile-trophy.vercel.app/?username=HasnainAfzaal&theme=tokyonight&no-frame=true&margin-w=15"/>

---

# 📌 Featured Projects

🖥️ Enterprise ERP System

🛒 POS Management System

📦 Inventory Management System

🌐 Business Website

🔐 Authentication & Role-Based System

⚡ REST API

📱 Flutter Mobile Application

---

# 🌍 Connect With Me

<p>

<a href="https://linkedin.com/in/YOUR_LINKEDIN">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin"/>
</a>

<a href="mailto:hasnainafzaal64@gmail.com">
<img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail"/>
</a>

</p>

---

<div align="center">

### "Building software that solves real business problems with modern engineering practices and AI-assisted development."

⭐ If you like my work, consider following my journey.

</div>
